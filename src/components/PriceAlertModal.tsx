          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-display font-bold text-white flex items-center gap-2">
                {prefilledAlert?.id ? 'Edit Price Alert' : 'Create Real-Time Price Alert'}
              </h3>
              <p className="text-xs text-slate-300">
                Get notified instantly via In-App, WhatsApp, or SMS when market prices hit your target.
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSaveAlert} className="p-5 sm:p-6 overflow-y-auto space-y-5 text-slate-800">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-36 overflow-y-auto pr-1">
              {CROP_PRICE_PROFILES.map((crop) => {
                ...
                return (
                  ...
                  >
                    <div className="font-bold truncate">{crop.name}</div>
                    <div className="text-[10px] text-slate-500 flex items-center justify-between mt-1">
                      <span>₹{crop.currentKisan} {crop.unit}</span>
                      <span className="text-[9px] text-emerald-700 font-semibold">{crop.category.split(' ')[0]}</span>
                    </div>
                  </button>
                );
              })}
          ...
          <div className="p-3.5 rounded-2xl bg-slate-50 gradient-border-organic border-0 flex flex-wrap items-center justify-between gap-3 text-xs">
            <div className="flex items-center gap-2">
              ...
              <div>
                <span className="font-semibold text-slate-700">{cropProfile.name} ({cropProfile.variety})</span>
                <div className="text-[11px] text-slate-500">Mandi: {cropProfile.primaryMandi}</div>
              </div>
            </div>
            <div className="flex items-center gap-4 text-right">
              <div>
                <div className="text-[10px] text-slate-400 uppercase font-semibold">Direct Realization</div>
                <div className="font-bold text-emerald-800">₹{cropProfile.currentKisan} {unit}</div>
              </div>
              <div className="border-l border-slate-200 pl-3">
                <div className="text-[10px] text-slate-400 uppercase font-semibold">APMC Modal</div>
                <div className="font-bold text-amber-700">₹{cropProfile.currentMandi} {unit}</div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">
                Price Benchmark Basis
              </label>
              <div className="space-y-1.5">
                <button
                  type="button"
                  onClick={() => setPriceType('KISAN_DIRECT')}
                  className={`w-full p-2 rounded-xl text-xs font-semibold text-left border flex items-center justify-between ${
                    priceType === 'KISAN_DIRECT'
                      ? 'border-emerald-600 bg-emerald-50 text-emerald-950'
                      : 'border-slate-200 bg-white text-slate-600'
                  }`}
                >
                  <span>KisanMandi Direct (Farm-gate)</span>
                  <span className="font-bold text-emerald-700">₹{cropProfile.currentKisan} {unit}</span>
                </button>
                ...
                >
                  ...
                  <span className="font-bold text-amber-700">₹{cropProfile.currentMandi} {unit}</span>
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-emerald-50/70 border border-emerald-200 space-y-3">
            <div className="flex items-center justify-between">
              <label className="text-xs font-bold text-emerald-950 uppercase tracking-wider">
                3. Set Target Price ({unit})
              </label>
              <div className="text-xs font-bold text-emerald-800">
                {priceDiff >= 0 ? `+₹${priceDiff} (+${pctDiff}%)` : `₹${priceDiff} (${pctDiff}%)`} vs current
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="w-10 h-10 rounded-xl bg-white border border-slate-300 text-slate-700 font-bold text-lg hover:bg-slate-50 flex items-center justify-center shrink-0"
              >
                -
              </button>
          ...
          <div className="space-y-2">
            <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider">
              4. Notification Dispatch Channels
            </label>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {[
                { id: 'in_app', label: 'In-App Alert', icon: <Bell className="w-4 h-4" /> },
                { id: 'whatsapp', label: 'WhatsApp', icon: <MessageSquare className="w-4 h-4 text-emerald-600" /> },
                { id: 'sms', label: 'SMS Gateway', icon: <Smartphone className="w-4 h-4 text-blue-600" /> },
                { id: 'push', label: 'Push Alert', icon: <Sparkles className="w-4 h-4 text-amber-500" /> },
              ].map((c) => {
            ...
            {(channels.includes('whatsapp') || channels.includes('sms')) && (
              <div className="mt-2">
                <input
                  type="text"
                  placeholder="Enter Mobile / WhatsApp Number (+91 ...)"
                  value={phoneOrEmail}
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">
              5. Alert Memo / Action Note (Optional)
            </label>
            <input
              type="text"
              placeholder="e.g., Release 50 MT cold storage batch, or execute B2B contract"
              value={note}
          ...
          <div className="pt-3 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-3">
            <button
              ...
              className="w-full sm:w-auto px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold transition-colors flex items-center justify-center gap-1.5"
            >
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto">
              <button
                type="button"
                onClick={closeCreateAlertModal}
                className="w-1/2 sm:w-auto px-4 py-2.5 rounded-xl border border-slate-300 text-slate-700 text-xs font-bold hover:bg-slate-100"
              >
                Cancel
              </button>
              <button
                type="submit"
                className="w-1/2 sm:w-auto px-6 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold shadow-md shadow-emerald-600/20 transition-all flex items-center justify-center gap-1.5"
              >
                {successSaved ? (
                  <>
                  ...
                  <>
                    ...
                    <span>{prefilledAlert?.id ? 'Update Alert' : 'Activate Price Alert'}</span>
                  </>
                )}
              </button>
            </div>
