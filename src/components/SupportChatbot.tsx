...
export const SupportChatbot: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      text: 'Hello! I am the KisanMitra AI Support Bot. How can I help you today?',
      sender: 'bot',
      timestamp: new Date()
    }
  ]);
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const newBotMsg: Message = {
        ...
        text: data.text || "I'm having trouble connecting right now. Please try again later or call our human support team at 1800-123-4567.",
        sender: 'bot',
        timestamp: new Date()
      };
      ...
      const errorMsg: Message = {
        id: (Date.now() + 1).toString(),
        text: "Sorry, I encountered an error. Please contact 1800-123-4567 for immediate assistance.",
        sender: 'bot',
        timestamp: new Date()
      };
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="fixed bottom-6 right-6 z-50 p-4 bg-emerald-600 hover:bg-emerald-700 text-white rounded-full shadow-2xl hover:shadow-emerald-500/30 transition-all hover:-translate-y-1 group"
        title="Help & Support"
      >
    ...
    <div className={`fixed right-4 sm:right-6 z-50 flex flex-col bg-gradient-to-br from-white/80 via-slate-50/50 to-emerald-50/40 backdrop-blur-xl hover:border-emerald-400/60 hover:shadow-[0_8px_32px_rgba(52,211,153,0.15)] hover:-translate-y-0.5 border border-emerald-200/60 rounded-2xl shadow-2xl transition-all duration-300 ease-in-out ${isMinimized ? 'bottom-6 w-72 h-14' : 'bottom-6 w-[350px] sm:w-[400px] h-[550px] max-h-[85vh]'}`}>
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-emerald-700 to-emerald-900 rounded-t-2xl text-white">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-bold text-sm leading-tight">Customer Support</h3>
      ...
      {!isMinimized && (
        <>
          ...
          <div className="bg-amber-50 border-b border-amber-100 px-4 py-2 flex items-center justify-center gap-2">
            ...
            <span className="text-xs font-semibold text-amber-800">Toll-Free Help: 1800-123-4567</span>
          </div>
          ...
          <div className="p-3 bg-white border-t border-slate-100 rounded-b-2xl">
            <form onSubmit={handleSend} className="relative flex items-center">
              ...
              <button
                ...
                className="absolute right-1.5 p-1.5 bg-emerald-600 hover:bg-emerald-700 disabled:bg-slate-300 text-white rounded-lg transition-colors"
              >
                <Send className="w-4 h-4" />
              </button>
            </form>
            <div className="text-center mt-2">
              <span className="text-[10px] text-slate-400">Powered by KisanMitra AI</span>
            </div>
          </div>
        </>
      )}
